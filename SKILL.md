...
细则按成品翻：长文 `article-workflow.md`；知识卡 `card-workflow.md`；若这一张本身就是在清点步骤/成品，可参考 `explainer-workflow.md` 的排法写进同一条 prompt。再读 `style-dna.md`。拼完 prompt 过后 `qa-checklist.md`。
...
示范图路径在 `assets/character/` 与 `assets/style-lock/`。设定说明在 `examples/jinger/`。公开填写示例在 `examples/ania/`。
