...
- 知识卡片默认 6–8 张，最多 9 张（也可单卡深讲）。流水线：原文 → 内容分析 → 版式模具 → 风格（单强调色）→ 角色包（edit 优先，可不出人）→ 生图 →（二期可选程序排字）→ 水印与 QA。气质对齐 `knowledge-card-editorial-3x4.png`。全身/半身/`none` 按排版与文字互动选择。禁止「降质求通」交付。
- 3:4 信息图页不强制出角色：表格、流程、对比、文件树已经自洽时，`character_budget: none` 是首选；角色出场时通常只占 12%–22%，必要时最多 28%，且必须通过手持、指向、贴入、检查、连接箭头等动作和文字发生关系（职能测试：去掉人信息关系仍完全成立则不出人）。
