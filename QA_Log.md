...
### 问题
1 发型统一叫两侧垂落小侧马尾= bust；2/3 可改双色+模具+职能测试；4 通道先写宽泛「需能调生图模型」，再写 Codex 优先与 Cursor 指引。

### 答案核心
- spec/character/QA/prompt：两侧垂落小侧马尾 + 粉→棕黑 + 眼下痣 + 耳机。
- card-style：单强调色硬纪律；layout：版式模具库；QA：职能测试+色纪律。
- SKILL：生图能力要求（多模型举例）+ 直连优先 + Cursor MCP 简要指引。

### 相关引用
- `SKILL.md`、`card-style-system.md`、`card-layout-system.md`、`character-spec.md`（jinger-v2）

## 2026-08-11

### 问题
下载「机器坏人的AI日常」图文笔记实图，研究特点/优势，反推 prompt 与生图流程约束；我们知识卡如何借鉴。

### 答案核心
- IP 极简（圆头+一撮发+粗黑线），一致性靠「简化到几乎画不歪」；配色黑白+蓝（品牌）+粉（强调）。
- 版式模板化：封面钩子 / 双格漫画 / 问答+中央隐喻物 / 错对对比+Tips / 编号系列；字有层级：顶标题→中对白→底结论。
- 角色永远有信息职能；大面积留白；表格类更像「人图+排字」混合。
- 可借：双强调色纪律、出场职能测试、版式模板库、IP 记忆点极简化锁死；不照搬对方简笔风（我们是像素编辑卡）。
...
### 问题
拆解某小红书博主图文（疑似 skill 生成）：为何文字与 IP 搭配好、互动有效、信息清晰、字密度够且角色不冗余、每图约 2 色？我们知识卡如何借鉴？

### 答案核心
- 小红书页被风控挡了，无法逐帖截图；结合描述与同类开源 skill（如 xfc-ip-all-png-template）归纳方法，非点名实锤。
- 好图共性：固定层极稳（画风+≤2 色+角色公式）/ 角色是信息功能不是装饰 / 一图一观点 / 字从图解结构长出 / 色只给重点。
- 我们可借：双色硬约束、角色按「信息功能」定大小与裁切、先图解结构再塞人、禁止装饰性出场；不必照搬对方默认「人极小+字极少」（对方可能是文章配图系；我们是知识卡系，密度可更高但纪律要更严）。

### 相关引用
- https://github.com/uncle11/xfc-ip-all-png-template
- `references/card-workflow.md` 等
...
### 问题
README 开头仍写正文默认白底迷你剧场；要加长文/知识卡使用场景；Mina 换成今晚的 Ania；「和别人的差别」改名；删掉奇怪的试用角色那句，换成介绍 skill 是什么、为什么做、能生成什么。

### 答案核心
- 去掉「默认」版式表述：正文按这一张要讲什么来画。
- 长文多用于公众号/推特长文/小红书长文；知识卡多用于小红书图文笔记。
- `examples/example-character`（Mina）换成 `examples/ania`（全身/半身/三视图+设定）。
- README 开头改成 skill 介绍；「和其他类似 skill 的区别」。

### 相关引用
- `README.md`、`examples/ania/`、`SKILL.md`、`LICENSE-ASSETS`
